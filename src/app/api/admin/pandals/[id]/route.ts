...
import { deleteItem, mergePandals, moderate } from "@/server/moderation";
...
export const PATCH = handle(async (req, ctx: Ctx) => {
  ...
  await moderate("pandals", id, moderationSchema.parse(await readJson(req)));
  return json({ ok: true });
});
